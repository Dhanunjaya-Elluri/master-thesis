...
"""Graph Attention Layers for Pyraformer model."""
...
from Pyraformer.pyraformer.hierarchical_mm_tvm import graph_mm as graph_mm_tvm
...
def get_q_k(
    ...
    """
    Generates a query-key mask matrix for use in Pyramidal Attention Module (PAM) with TVM.

    This function creates a mask that determines which keys are attended to by each query.
    The mask is used for graph-based efficient computation of attention.

    Args:
        input_size (int): The size of the input sequence.
        window_size (int): The window size for attention.
        stride (int): The stride size for pyramidal downsampling.
        device (torch.device): The device on which the tensor will be created.

    Returns:
        torch.Tensor: A 2D integer tensor representing the query-key mask matrix.
    """
    ...
    # Find the corresponding k for each q in the sequence in the order of intra-layer, lower-layer, and upper-layer
    # level one
    for i in range(input_size):
        mask[i, 0:window_size] = i + torch.arange(window_size) - window_size // 2
        # When window is at the right end of the sequence, comment it out
    ...
    for i in range(second_length):
        ...
        # When the window is at the left end of the sequence, it is set to -1
        mask[input_size + i, mask[input_size + i] < input_size] = -1
        # When the window is at the right end of the sequence, it is set to -1
...
def get_k_q(q_k_mask: torch.Tensor) -> torch.Tensor:
    """
    Generates a key-query index from a given query-key index for PAM-TVM.

    This function inverts the mapping from queries to keys, creating a mask for efficient
    computation in the attention mechanism.

    Args:
        q_k_mask (torch.Tensor): The query-key mask tensor.

    Returns:
        torch.Tensor: The key-query mask tensor.
    """
...
def get_mask(
    input_size: int, window_size: int, inner_size: int, device: torch.device
) -> Tuple[torch.Tensor, List[int]]:
    """
    Generates an attention mask for PAM-Naive implementation.

    This function creates a binary mask for attention, defining allowed connections
    in multi-scale attention layers.

    Args:
        input_size (int): The size of the input sequence.
        window_size (int): The window size for attention.
        inner_size (int): The size of the inner attention window.
        device (torch.device): The device on which the tensor will be created.

    Returns:
        Tuple[torch.Tensor, List[int]]: A tuple containing the attention mask and the size of all layers.
    """
...
class GraphSelfAttention(nn.Module):
    """
    A PyTorch module implementing Graph Self-Attention.

    This module utilizes graph-based mechanisms to efficiently compute attention over sequences,
    especially suitable for long sequences.

    Args:
        opt: Configuration options containing model dimensions, number of heads, dropout rates, etc.
    """
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for the Graph Self-Attention layer.

        Args:
            hidden_states (torch.Tensor): Input tensor of shape (batch_size, sequence_length, d_model).
...
class NormalSelfAttention(nn.Module):
    """
    A PyTorch module implementing Normal Multi-Head Self-Attention.

    This standard implementation of self-attention computes attention weights
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for the Normal Self-Attention layer.
...
class ProbSparseAttention(nn.Module):
    """
    A PyTorch module implementing Probabilistic Sparse Self-Attention.

    This module uses a sparsity-inducing mechanism to select a subset of key-query pairs
    for attention, which reduces computation especially for long sequences.
    ...
    def _prob_QK(
        ...
        """
        Calculates the probabilistic sparse query-key attention scores.

        This method computes attention scores based on a subset of key-query pairs,
        selecting the top queries based on sparsity measurement.

        Args:
            Q (torch.Tensor): The query tensor of shape (B, H, L_Q, D).
            K (torch.Tensor): The key tensor of shape (B, H, L_K, D).
            sample_k (int): Number of keys to sample for each query.
            n_top (int): Number of top queries to select, typically c * log(L_Q).

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: A tuple containing the reduced query-key attention scores
            and the indices of the top queries.
        """
        # Q [B, H, L, D]
        B, H, L_K, E = K.shape
        _, _, L_Q, _ = Q.shape
    ...
    def _get_initial_context(self, V: torch.Tensor, L_Q: int) -> torch.Tensor:
        """
        Initializes the context tensor for the attention mechanism.

        This method creates an initial context based on the value tensor V by averaging its values.

        Args:
            V (torch.Tensor): The value tensor of shape (B, H, L_V, D).
            L_Q (int): The length of the query sequence.

        Returns:
            torch.Tensor: The initial context tensor expanded to match the query sequence length.
        """
        B, H, L_V, D = V.shape
        V_sum = V.mean(dim=-2)
    ...
    def _update_context(
        ...
        """
        Updates the context tensor with attention information.

        This method updates the given context tensor based on the attention scores and the value tensor.

        Args:
            context_in (torch.Tensor): The context tensor to update, of shape (B, H, L_Q, D).
            V (torch.Tensor): The value tensor of shape (B, H, L_V, D).
            scores (torch.Tensor): The attention scores of shape (B, H, n_top, L_K).
            index (torch.Tensor): The indices of the top queries.
            L_Q (int): The length of the query sequence.

        Returns:
            torch.Tensor: The updated context tensor.
        """
        B, H, L_V, D = V.shape
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for the Probabilistic Sparse Self-Attention layer.
...
def parsing() -> argparse.Namespace:
    """
    Parses command-line arguments needed for the graph self-attention.
...
def test_NSA(args: argparse.Namespace, input_len: int):
    """
    Tests the Normal Self-Attention (NSA) layer for time and CUDA memory consumption.

    Args:
        args (argparse.Namespace): Parsed arguments containing model and test configurations.
        input_len (int): Length of the input sequence for the test.
    """
...
def test_GSA(args: argparse.Namespace, input_len: int):
    """
    Tests the Graph Self-Attention (GSA) layer for time and CUDA memory consumption.
...
def test_PSA(args: argparse.Namespace, input_len: int):
    """
    Tests the Probabilistic Sparse Self-Attention (PSA) layer for time and CUDA memory consumption.
